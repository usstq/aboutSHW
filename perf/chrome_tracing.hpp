...
struct TscCounter {
    ...
    TscCounter() {
        ...
        std::call_once(flag, [&]() {
            ...
            std::cout << "[PET_PROFILE] tsc_ticks_per_second = " << tsc_ticks_per_second << std::endl;
...
struct ProfilerFinalizer {
    ...
    void finalize() {
        ...
        dump_file_over = true;
        not_finalized = false;
        std::cout << "[PET_PROFILE] Dumpped " << total_size / (1024 * 1024) << " (MB) to " << dump_file_name
                  << std::endl;
    }
    ...
    int register_manager(ProfilerBase* pthis) {
        ...
        ss << "[PET_PROFILE] #" << serial_id << "(" << pthis << ") : is registed." << std::endl;
        std::cout << ss.str();
...
public:
    Profiler() {
        const char* str_enable = std::getenv("PET_PROFILE");
        if (!str_enable)
            str_enable = "0";
    ...
    void save(std::ofstream& fw, detail::TscCounter& tsc) override {
        ...
        all_data.clear();
        std::cout << "[PET_PROFILE] #" << serial << "(" << this << ") finalize: dumpped " << data_size << std::endl;
    }
...
#    define PROFILE(var_name, ...)                           \
        auto var_name = Profiler::startProfile(__VA_ARGS__); \
        (void)var_name;

#    define PROFILE2(var_name, ...) var_name = Profiler::startProfile(__VA_ARGS__);
