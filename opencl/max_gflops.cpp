...
int main(void) {
    ...
    CLkernels kernel(R"CLC(
        ...
        __kernel void fill_id(
            ...
            ids[i*16 + 7] = intel_get_dual_subslice_id();
            ids[i*16 + 8] = intel_get_eu_id();
            ids[i*16 + 9] = intel_get_eu_thread_id();
            ...
            ids[i*16 + 10] = intel_get_cycle_counter();
            ...
            ids[i*16 + 11] = intel_get_cycle_counter() - ids[i*16 + 9];
    ...
    auto evt = kernel.call("fill_id", enqArgs, ids.to_gpu(), info.to_gpu(), K);
    evt.wait();
    ...
    struct EUWork {
        ...
        void dump(ChromeTraceDumpper & dumpper) {
            ...
            ss << "EU_" << eu_id << "." << eu_slot_id << "(" << group_id0 << "," << group_id1 << ").";
