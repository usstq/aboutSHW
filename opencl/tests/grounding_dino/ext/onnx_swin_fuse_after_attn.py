...
class MyModelONNX(nn.Module):
    ...
    def forward(self, attn_windows: torch.Tensor, query_4d: torch.Tensor):
        return StubPermuteRollCrop.apply(attn_windows, query_4d, self.shift_size, self.window_size, self.embed_dims)
...
def export_onnx(onnx_model, attn_windows_shape, query_shape):
    print(f'convert onnx model to "{onnx_path}"...', end='')
    np.random.seed(324)
    torch.manual_seed(32)

    attn_windows = Variable(torch.randn(attn_windows_shape))
    query = Variable(torch.randn(query_shape))

    with torch.no_grad():
        torch.onnx.export(onnx_model, (attn_windows, query), onnx_path,
                        input_names=['attn_windows', 'query', ],
                        output_names=['out'])

    print('done')

## Test
def test_ov(model_path, attn_windows, query, times=100):
    core = ov.Core()
...
for shift_size in SHIFT_SIZE:
    ...
    for head_dims, hw in zip(HEAD_DIMS, HW):
        ...
        h_pad = (hw[0] + WINDOW_SIZE - 1) // WINDOW_SIZE * WINDOW_SIZE
        w_pad = (hw[1] + WINDOW_SIZE - 1) // WINDOW_SIZE * WINDOW_SIZE
        win_no = h_pad // WINDOW_SIZE * (w_pad // WINDOW_SIZE)
        attn_shape = [win_no, WINDOW_SIZE * WINDOW_SIZE, head_dims]
        ...
        export_onnx(copy.deepcopy(onnx_model), attn_windows_shape=attn_shape, query_shape=query_shape)

        attn_windows = np.random.random(attn_shape).astype(dtype=np.float32)
        query = np.random.random(query_shape).astype(dtype=np.float32)
        print(f'test "{onnx_path}"...', end='')
        cur = test_ov(onnx_path, attn_windows=attn_windows, query=query, times=ov_test_times)
        ref = onnx_model(torch.from_numpy(attn_windows), torch.from_numpy(query)).detach().numpy()
