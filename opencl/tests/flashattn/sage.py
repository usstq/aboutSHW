...
#define VNNI_WIDTH_HALF 2
#define VNNI_WIDTH_U8 4

#define REG_K_HALF (SystolicDepth * VNNI_WIDTH_HALF)
...
extern "C" _GENX_MAIN_ void cm_sdpa(
    ...
    for(int kv_pos = 0; kv_pos < kv_stop; kv_pos += kv_step) {
        ...
        //# load V into SLM as dpas-B tile (shared by all hw within same WG)
        {
            ...
            if (kv_pos > 0) cm_barrier();
            {
                if (wg_local_id < WG_SIZE/2) {
                    ...
                    for(int k = REG_K_HALF*(wg_local_id-WG_SIZE/2); k < head_size; k += REG_K_HALF*(WG_SIZE/2)) {
        ...
        matrix<half, 2*REG_M, REG_K_HALF> P;
        ...
        if (kv_pos == 0) {
            ...
            matrix<half, REG_K_HALF/2, REG_N*2> Vmat;
            uint offset = o_offset;
            #pragma unroll
            for(int k = 0, ri = 0; k < head_size; k += REG_K_HALF, ri += 2) {
