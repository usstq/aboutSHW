...
namespace ov {
    ...
    template <typename T>
    struct precision_of {
        static constexpr ov::Type_t value = ov::Type_t::dynamic;
    };

    template <>
    struct precision_of<float> {
        static constexpr ov::Type_t value = ov::Type_t::f32;
    };

    template <>
    struct precision_of<int32_t> {
        static constexpr ov::Type_t value = ov::Type_t::i32;
    };

    template <>
    struct precision_of<uint8_t> {
        static constexpr ov::Type_t value = ov::Type_t::u8;
    };

    template <>
    struct precision_of<float16_t> {
        static constexpr ov::Type_t value = ov::Type_t::f16;
    };
...
#define PLAINTENSOR_RANK_MAX 8
    ...
    struct PlainTensor {
        ...
        [[nodiscard]] ov::Type_t get_precision() const {
            return m_dt;
        }
        ...
        [[nodiscard]] std::string repr(int max_total_lines = 16, int lines_per_row = 1) const {
            ...
            for (i = 0; i < sz && max_total_lines > 0; i++) {
                ...
                if (cur_row_lines_left > 0) {
                    if (m_dt == ov::Type_t::f32) {
                        ss << (ptr<float>())[i] << ",";
                    }
                    else if (m_dt == ov::Type_t::f16) {
                        ss << (ptr<float16_t>())[i] << ",";
                    }
                    else if (m_dt == ov::Type_t::i32) {
                        ss << (ptr<int32_t>())[i] << ",";
                    }
                    else if (m_dt == ov::Type_t::i8) {
                        ss << (ptr<int8_t>())[i] << ",";
                    }
                    else if (m_dt == ov::Type_t::u8) {
                        ss << (ptr<uint8_t>())[i] << ",";
                    }
                    else if (m_dt == ov::Type_t::boolean) {
