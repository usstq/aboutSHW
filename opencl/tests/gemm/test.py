...
def test_v2():
    ...
    if 1:
        ...
        devinfo = cl.dev_info()
        xecores = devinfo["CL_DEVICE_MAX_COMPUTE_UNITS"] // 16
        ...
        if N < M:
            ...
            if block_n % s0 == 0:
                ...
                if s0 * 1.5 < block_n:
                    rem = block_n % s0
                    slice_no = (block_n + s0 - 1) // s0 - (s0 - rem)
                    assert slice_no >= 0, f'{block_n=} {slice_no=} {s0=}'
                    slice = -s0
                    slice_no = -slice_no
        else:
            # TODO: big weight matrix?
            pass
    print(f'{block_m=} {block_n=} {slice=} {slice_no=}')
    kernels.enqueue("gemm_nocopy", [M // BLOCK_WG_M * N // BLOCK_WG_N * SG_N, SG_M], [SG_N, SG_M], tA, tB_trans, tC, M, N, K, K, K, N, slice_no, slice)
    cl.finish()
