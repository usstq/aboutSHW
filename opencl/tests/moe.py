...
def test_mm(M, K, N, K_group_size, w_dtype):
    ...
    if w_dtype == cl.onednn_dtype.u4:
        ...
        ocl_kernels = cl.kernels(mlp_sources, f"-DSUBGROUP_NUM={SUBGROUP_NUM} -DSUBGROUP_SIZE={SUBGROUP_SIZE} -D GROUP_SIZE={GROUP_SIZE} -D INTERMEDIATE_SIZE={INTERMEDIATE_SIZE} -D HIDDEN_SIZE={HIDDEN_SIZE} -D MAX_TOPK={MAX_TOPK}", "./dump")
