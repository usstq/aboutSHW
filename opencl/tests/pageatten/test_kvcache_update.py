...
class pa_kvcache_update_cm:
    def __init__(self, num_kv_heads, k_head_size, v_head_size, block_size):
        ...
        self.kernels = cl.kernels(src,
                      ...
                      f" -DADJUSTED_K_HEAD_SIZE={k_head_size}"
                      f" -DADJUSTED_V_HEAD_SIZE={v_head_size}"
    ...
    def __call__(self, key:torch.Tensor,
        ...
        t_key_cache = cl.tensor(key_cache.to(torch.float16).detach().numpy())
        t_value_cache = cl.tensor(value_cache.to(torch.float16).detach().numpy())
