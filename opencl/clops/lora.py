...
class LORA_2ND:
    def __init__(self, rank, input_state, output_state, gemma_sg_BK, gemma_sgK, gemmb_sgN, use_ref = False):
        ...
        self.cl_kernels_opt = kernel_cache(lora_kernel_2nd, options)
        ...
        if use_ref == False:
            ...
            print(f'| [GEMMA]: GWS:{self.gemma_gws}, LWS:{self.gemma_lws} SG_BK:{gemma_sg_BK} SGK:{gemma_sgK}')
            print(f'| [GEMMB]: GWS:{self.gemmb_lws}, LWS:{self.gemmb_lws} SGN:{gemmb_sgN}')
            print(f'----------------------------------------------------------------------------------------------------------------------------------')
...
def test_2ND(input_state, rank, output_state, gemma_sgK = 8, gemma_sg_BK = 32, gemmb_sgN = 16, check_acc = False):
    cl.profiling(True)
    SG_SZ = 16
    vRANGE = 1
    if check_acc:
        ...
        REPEAT = 200
    gemma_sgK = min(gemma_sg_BK, 1024//rank)
    ...
    if 1:
        for out_state in (256, 512, 1024, 2048, 1536, 3072, 3840, 15*16,  17*16, 7*16, 11*16, 13*16):
            for input_state in (1024, 1536, 2048, 2560, 3072, 3840, 4096, 7*16, 11*16, 13*16, 15*16, 12*16,17*16):
                for rank in range(16//16, 32//16, 64//16):
                    for sgk in range(1, 17):
                        for bk in (16, 32, 48, 64, 80, 96, 112):
                            test_2ND(input_state, rank*16, out_state, sgk, bk, 16, True)
        for rank in [64]:
            for out_state in [512, 1536, 3840, ]:
                test_2ND(1536, rank, out_state, gemma_sgK = 4, gemma_sg_BK=32, gemmb_sgN = 16, check_acc=True)
    else:
        for rank in [64]:
            for out_state in [512, 1536, 3840]:
                test_2ND(1536, rank, out_state, gemma_sgK = 8, gemma_sg_BK=32, gemmb_sgN = 32)
    # for rank in [512]:
    #     for out_state in [4096]:
    #         for input_state in [4096]:
    #             test(1536, rank, out_state, gemma_sgK = 2, gemma_sg_BK=64, gemmb_sgN = 16)
