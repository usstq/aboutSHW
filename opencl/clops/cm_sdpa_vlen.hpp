...
#else
    sdpa_kernel<false, num_heads, num_kv_heads, head_size>(
                                slm_K,
                                slm_V,
                                wg_local_id,
                                local_size,
...
extern "C" _GENX_MAIN_ void cm_sdpa_single_causal(
    int seqlen,
#if USE_LSC == 1
    ...
    kv_start = 0;
    kv_seq_len = seqlen;
    q_start = (wg_id) * wg_seq_len + wg_local_id * q_step;
    q_len = q_step;
...
#else
    // qkv fused
    uint num_total_heads = num_heads + num_kv_heads * 2;
    ...
    sdpa_kernel<false, num_heads, num_kv_heads, head_size, 1>(
                                slm_K,
                                slm_V,
                                wg_local_id,
                                local_size,
