...
#        endif
        ...
        barrier(CLK_LOCAL_MEM_FENCE);
        {
            ...
            SOFTMAX_ACCUMULATOR_TYPE total_max = SOFTMAX_ACCUMULATOR_MAX_FUNC(max_val_prev, max_val_cur);
            qk_max_new = total_max;
...
#        endif
                for (uint seq_idx = 0; seq_idx < seq_idx_end; seq_idx++) {
                    ...
                    SOFTMAX_ACCUMULATOR_TYPE update_factor = native_exp(sub_group_broadcast(max_val_prev, seq_idx) - total_max);
                    SOFTMAX_ACCUMULATOR_TYPE updated_total_exp_sum = exp_sum_prev * update_factor + exp_sum_cur;
                    if (start_partition_idx > 0) {
                        OUTPUT_TYPE updated_prev_res = TO_SOFTMAX_ACCUMULATOR_TYPE(output_acc[seq_idx]) * update_factor;
                        ;
                        // if (sgid == 0 && sglid == 0) {
                        //     printf("updated_prev_res[%d]=%f <- output_acc=%f * %f; acc_output_res=%f *= %f;  @%d,%d\n", seq_idx,
                        //         updated_prev_res, output_acc[seq_idx], updated_exp_sum_prev,
                        //         acc_output_res[seq_idx], updated_exp_sum_cur,
                        //         target_seq_dim, start_partition_idx);
                        // }
                        acc_output_res[seq_idx] += updated_prev_res;
                    }
                    output_acc[seq_idx] = acc_output_res[seq_idx];
                    // if (sgid == 0 && sglid == 0) {
                    //     printf("output_acc[%d]=%f, exp_sum=%f -> %f, max=%f -> %f;  @%d,%d\n", seq_idx,
                    //         output_acc[seq_idx], slm_exp_sum_prev[seq_idx], updated_total_exp_sum, slm_max_val_prev[seq_idx], total_max,
                    //         target_seq_dim, start_partition_idx);
                    //     printf("\n");
                    // }
                    if (sgid == 0 && sglid == 0) {
