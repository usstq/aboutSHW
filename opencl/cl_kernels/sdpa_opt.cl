...
#if 1
#define SOFTMAX_ACCUMULATOR_TYPE           half
#define SOFTMAX_ACCUMULATOR_VAL_MAX        HALF_MAX
...
#define TO_SOFTMAX_ACCUMULATOR_TYPE(v)     convert_half(v)
#define TO_SOFTMAX_ACCUMULATOR_TYPE_SAT(v) convert_half(v)
#define AS_SOFTMAX_ACCUMULATOR_TYPE(v)     as_half(v)
#define SOFTMAX_ACCUMULATOR_MAX_FUNC       fmax
#define SOFTMAX_ACCUMULATOR_MIN_FUNC       fmin
#define SOFTMAX_ACCUMULATOR_ABS_FUNC       fabs
#define SOFTMAX_ACCUMULATOR_TYPE_SIZE      2
...
//                                            seq_len ATTN_MASK_BUFFER ATTN_SCALE_BUFFER PA_BUFFERS);
        ...
        if (seq_len <= target_seq_idx) { // apply casual mask
    ...
    #define QUERY_VEC                   MAKE_VECTOR_TYPE(INPUT1_TYPE, TARGET_SEQ_LEN_BLOCK_SIZE)
                    ...
                    unroll_for(uint key_row_idx = 0; key_row_idx < TARGET_SEQ_LEN_BLOCK_SIZE; key_row_idx++) {
                        ...
                        unroll_for(uint i = 0; i < SUBGROUP_SIZE; i++) {
                            qk_acc[key_row_idx] = fma(sub_group_broadcast(key_vals, i), queries_vec[i], qk_acc[key_row_idx]);
                            // if (pw->group_id0==0 && pw->group_id1==0 && pw->group_id2==0 && pw->sub_group_id==0) {
                            //         mad_counts[0]++;
                            // }
                        }
    ...
    #define key_vals key_vec[key_row_idx]
            ...
            slm_qk_max_vals[sgid * SUBGROUP_SIZE + sglid] = SOFTMAX_ACCUMULATOR_VAL_MIN;
        }
        barrier(CLK_LOCAL_MEM_FENCE);
        {
            ...
            if (partition_seq_len == SEQ_LEN_PARTITION_SIZE) {
                ...
                for (uint seq_len = seq_len_start; seq_len < seq_len_start + (SEQ_LEN_PARTITION_SIZE / SG_SCALE_FACTOR); seq_len += SUBGROUP_SIZE) {
                    ...
                    unroll_for(uint i = 0; i < SUBGROUP_SIZE; i++) {
                        ...
                        // }
                        unroll_for(uint seq_idx = 0; seq_idx < TARGET_SEQ_LEN_BLOCK_SIZE; seq_idx++) {
                            acc_output_res[seq_idx] = fma(sub_group_broadcast(qk_val[seq_idx], i), value_val, acc_output_res[seq_idx]);
                            // if (pw->group_id0==0 && pw->group_id1==0 && pw->group_id2==0 && pw->sub_group_id==0) {
                            //         mad_counts[2]++;
                            // }
                        }
                ...
                if (seq_len_leftovers_start != seq_len_end) {
                    ...
                    for (uint seq_len_idx = 0; seq_len_idx < partition_seq_len - seq_len_leftovers_start; seq_len_idx++) {
                        ...
                        for (uint seq_idx = 0; seq_idx < TARGET_SEQ_LEN_BLOCK_SIZE; seq_idx++) {
                            acc_output_res[seq_idx] = fma(sub_group_broadcast(qk_val[seq_idx], seq_len_idx), value_val, acc_output_res[seq_idx]);
                            // if (pw->group_id0==0 && pw->group_id1==0 && pw->group_id2==0 && pw->sub_group_id==0) {
                            //         mad_counts[4]++;
                            // }
                        }
