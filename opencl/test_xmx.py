...
__attribute__((intel_reqd_sub_group_size(8)))
__kernel void XMX_8x8_PREPACKED(__global half * A, __global half * B, __global float * C,  __global int *K) {
    int k_loop = (*K)/ 16;
    float8 acc = 0.f;
...
kernels.enqueue("XMX_8x8_PREPACKED", [8],[8], tA_PACK, tB_PACK, tC, tK)
compare(C, tC.numpy())
